#!/usr/bin/env node
/**
 * S2-6j V1 REGRESSION 编排器（§25 / §26 / §27 / §28）。
 *
 * 在全新隔离 state（.tmp/s2-6j-state，原生 0001→0004，无 0005）上实跑：
 *   P0   原生迁移 + 校验 migrations=4 / 0004 索引
 *   P1   NORMAL worker（PORT 8795，非 8787）
 *   P2   frozen regression 8 套（340）
 *   P3   S2-6i 主集成（80）
 *   P6   S2-6j 主集成（>=60）
 *   P4/7 S2-6i atomic MODE1 + S2-6j atomic MODE1（fault worker 1）
 *   P5/8 S2-6i atomic MODE2 + S2-6j atomic MODE2（fault worker 2）
 *   P9   完整性 / teardown（FK 0 / integrity ok / catalog 6·83·238 / 业务表 0 / 8787 不动）
 *
 * 端口纪律（§29）：仅用 PORT（默认 8795），只 kill 监听该端口的 PID；8787（Signivra）绝不触碰、绝不 broad kill。
 */
...
/**
 * 递归删除隔离 state 目录。
 * 注意：node:fs 的 rmSync 在本环境被 WorkBuddy safe-delete shim 拦截，对含大量小文件的
 * D1 state 目录会 spawn 外部 trash 二进制并 ETIMEDOUT。故先尝试原生 rmSync，失败时
 * 回退到 OS 原生 `rm -rf`（Git Bash）/ `rd /s /q`（cmd），绕过 shim。
 */
function removeState() {
  ...
  try {
    ...
    try {
      execFileSync('rm', ['-rf', STATE], { stdio: 'ignore' });
    } catch {
      try {
        execFileSync('cmd.exe', ['/c', 'rd', '/s', '/q', STATE], { stdio: 'ignore' });
      } catch {
        /* 末路：保留 STATE，交由后续 d1 migrations apply 覆盖 */
      }
    }
  }
}
...
const CWD = resolve('.');
const PORT = Number(process.env.JHZY_TEST_PORT ?? 8795); // 绝不使用 8787（Signivra）
const BASE = `http://127.0.0.1:${PORT}`;
const STATE = resolve('.tmp/s2-6j-state');
const D1_DIR = join(STATE, 'v3', 'd1', 'miniflare-D1DatabaseObject');
const MANIFEST = join('.tmp', 's2-6j-manifest.json');
const TMP = resolve('.tmp');
const LOG = join(TMP, 'run_s2_6j.log');
const SUMMARY = join(TMP, 's2-6j-summary.json');
const FAULT_CFG_1 = resolve('wrangler.s2-6j-fault1.jsonc');
const FAULT_CFG_2 = resolve('wrangler.s2-6j-fault2.jsonc');
...
const results = { suites: [], s2_6i: {}, s2_6j: {}, static: {}, integrity: {}, teardown: {}, port: {} };
...
try {
  // ================= P0 全新隔离 state（原生迁移）=================
  log('===== P0: rebuild isolated S2-6j state (native wrangler migrations) =====');
  await killPort();
  ...
  if (mig.some((n) => n.includes('0005'))) throw new Error('UNEXPECTED_0005_MIGRATION');
  const idx = withDb((db) =>
  ...
  // ================= P6 S2-6j 主集成 =================
  log('===== P6: S2-6j MAIN integration (>=60) =====');
  ...
  results.s2_6j.main = { ...jCnt, total: jCnt.pass + jCnt.fail, exit: jCode, expect: 60 };
...
const s6jMainOk = results.s2_6j.main?.exit === 0 && (results.s2_6j.main?.fail ?? 1) === 0 && (results.s2_6j.main?.pass ?? 0) >= 60;
const s6jAtomOk = results.s2_6j.atomicity_mode1?.exit === 0 && (results.s2_6j.atomicity_mode1?.fail ?? 1) === 0
  && results.s2_6j.atomicity_mode2?.exit === 0 && (results.s2_6j.atomicity_mode2?.fail ?? 1) === 0;
const integrityOk =
...
log('');
log('=================== FINAL S2-6j REGRESSION SUMMARY ===================');
...
log(`S2-6j MAIN        = ${results.s2_6j.main?.pass}/${results.s2_6j.main?.total} exit=${results.s2_6j.main?.exit} (expect >=60)`);
...
const overall = frozenAllOk && frozenPass === 340 && frozenTotal === 340 && s6iOk && s6jMainOk && s6jAtomOk && integrityOk && !results.error ? 0 : 1;
