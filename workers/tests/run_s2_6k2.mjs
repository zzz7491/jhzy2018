#!/usr/bin/env node
/**
 * S2-6k2 V1 REGRESSION 编排器（Location Capture Foundation）。
 *
 * 在全新隔离 state（.tmp/s2-6k2-state，原生 0001→0005）上实跑：
 ...
 *   P2.6 S2-6k2 Location Capture 集成（location_integration.mjs，不计入 340）
 *   P3   S2-6i 主集成（80）
 *   P6   S2-6j 主集成（87）
...
const STATE = resolve('.tmp/s2-6k2-state');
const D1_DIR = join(STATE, 'v3', 'd1', 'miniflare-D1DatabaseObject');
const MANIFEST = join('.tmp', 's2-6k2-manifest.json');
const TMP = resolve('.tmp');
const LOG = join(TMP, 'run_s2_6k2.log');
const SUMMARY = join(TMP, 's2-6k2-summary.json');
const FAULT_CFG_1 = resolve('wrangler.s2-6k2-fault1.jsonc');
const FAULT_CFG_2 = resolve('wrangler.s2-6k2-fault2.jsonc');
...
const results = { suites: [], time_policy: {}, location: {}, s2_6i: {}, s2_6j: {}, static: {}, catalog: {}, integrity: {}, teardown: {}, port: {} };
...
try {
  // ================= P0 全新隔离 state（原生迁移）=================
  log('===== P0: rebuild isolated S2-6k2 state (native wrangler migrations 0001-0005) =====');
  await killPort();
  ...
  // ================= P2.6 S2-6k2 Location Capture 集成（不计入 340）=================
  log('===== P2.6: S2-6k2 LOCATION CAPTURE integration =====');
  await fixture('attendance', 'S2-6k2');
  const locOut = join(TMP, 'suite-S2-6k2.log');
  const locCode = await runCapture(NODE, ['tests/location_integration.mjs'], env, locOut);
  const locTxt = readFileSync(locOut, 'utf8');
  const locCnt = parseCounts(locTxt);
  results.location = { ...locCnt, total: locCnt.pass + locCnt.fail, exit: locCode, ok: locCode === 0 && locCnt.fail === 0 };
  log(`[P2.6] S2-6k2 LOCATION PASS=${locCnt.pass} FAIL=${locCnt.fail} TOTAL=${locCnt.pass + locCnt.fail} EXIT=${locCode}`);
  if (locCode !== 0) log('[P2.6] failures:\n' + locTxt.split('\n').filter((l) => l.includes('FAIL')).join('\n'));
  await fixture('teardown', 'post-S2-6k2');
...
const locOk = results.location?.exit === 0 && (results.location?.fail ?? 1) === 0;
...
log('');
log('=================== FINAL S2-6k2 REGRESSION SUMMARY ===================');
...
log(`S2-6k2 LOCATION    = PASS=${results.location?.pass} FAIL=${results.location?.fail} EXIT=${results.location?.exit} ok=${locOk}`);
...
const overall = frozenAllOk && frozenPass === 340 && frozenTotal === 340 && tpOk && locOk && s6iOk && s6jMainOk && s6jAtomOk && integrityOk && results.catalog?.ok && !results.error ? 0 : 1;
